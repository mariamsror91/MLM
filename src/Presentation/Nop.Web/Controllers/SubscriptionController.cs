...
public class SubscriptionController : BasePublicController
{
    ...
    public async Task<IActionResult> CustomerSubscriptions()
    {
        var id = (await _workContext.GetCurrentCustomerAsync()).Id;
        var subs = await _subscriptionService.GetActiveSubscriptionAsync(id);
    ...
    public async Task<IActionResult> Checkout(int packageId, bool isRenew = false)
    {
    ...
    public async Task<IActionResult> CheckoutComplete(int package = 0, double price = 0, bool isNew = false, bool isRenew = false, int activeSub = 0)
    {
        
        var customer = await _workContext.GetCurrentCustomerAsync();
        if(isNew)
        {
            var res = await _subscriptionService.CreateSubscriptionAsync(customer.Id, package, price);
            if(!isRenew)
